...
class AguToken::Test::SingletonClient < AguToken::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: AguToken::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class AguToken::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @agu_token = AguToken::Test::SingletonClient.instance
  end
